...
def train_loop():
    ...
    images_path = "../Datasets/HipMRI_study_complete_release_v1/semantic_MRs_anon"
    labels_path = "../Datasets/HipMRI_study_complete_release_v1/semantic_labels_anon"
    ...
    def get_subject_id(filename, is_img):
        basename = os.path.basename(filename)
        basename_no_ext = basename.split('.')[0]
        if is_img:
            subject_id = '_'.join(basename_no_ext.split('_')[:-1])  # Remove the last part after '_'
        else:
            subject_id = '_'.join(basename_no_ext.split('_')[:3])  # Remove the last part after '_'
        return subject_id

    # Create dictionaries mapping subject IDs to filenames
    image_dict = {get_subject_id(f, True): f for f in image_filenames}
    label_dict = {get_subject_id(f, False): f for f in label_filenames}
